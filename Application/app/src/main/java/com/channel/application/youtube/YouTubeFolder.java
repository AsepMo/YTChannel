...
public class YouTubeFolder {
    ...
    public static String TAG = YouTubeFolder.class.getSimpleName();
    ...
    public static void initYouTubeFolder()
    {
        File mFoldeAnalytics = new File(ZFOLDER_YOUTUBE_ANALYTICS);
        if (!mFoldeAnalytics.exists())
        {
            mFoldeAnalytics.mkdirs();
        }
        File mFolderDownload = new File(ZFOLDER_YOUTUBE_DOWNLOAD);
        if (!mFolderDownload.exists())
        {
            mFolderDownload.mkdirs();
        }
    }
